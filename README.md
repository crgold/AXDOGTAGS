...
LayerZero Labs' [example contracts](https://github.com/LayerZero-Labs/solidity-examples) and additionally includes all the
necessary setup to get started with Beam, as well as more improvements like contract verification and better configuration options under the hood.
...
- ExtendedONFT721 & ExtendedONFT721Upgradeable `*` (Enumerable ERC721 with on-chain royalties and batch transfers)
...
npx hardhat --network ethereum oftv2Send --target-network beam --qty 20 --local-contract MyUsdcProxyOFT --remote-contract MyUsdcOFT
```
